...
function Library:CreateWindow(config)
    ...
    --// Adds a command, button-style \\--
    local Window = {}
    Window.Commands = {}
    function Window:AddCommand(opts)
        local cmdName = opts.Name:lower()  -- normalize
    ...
    TextBox.FocusLost:Connect(function(enterPressed)
        ...
        elseif Window.Commands[text] then
            local cmd = Window.Commands[text]
            if cmd.Callback then
                cmd.Callback()
            end
            TextBox.Text = ""
        else
            print("'"..text.. "' is not recognized as an internal or external command or any elements.")
        end
    end)
    return Window
end

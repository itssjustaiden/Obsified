--[[
    Cats.lol UI Library, CMD version
    Continued version of VISUAL
    Created by 'fluffyfem' on discord
    V1.0
]]
...
local ismobile = not game:GetService('UserInputService').KeyboardEnabled
do
    function Utility:Log(Type, Message)
        Type = Type:lower()
        if Type == 'error' then error('[Cats.lol] err: ' .. Message) elseif Type == 'log' then print('[Cats.lol] ' .. Message) elseif Type == 'warn' then warn('[Cats.lol] warn: ' .. Message) end
    end
    ...
    function Utility:Create(InstanceName, Properties, Children)
        ...
        for Index, Property in next, Properties do Object[Index] = Property end
    ...
    function Utility:EnableDragging(Frame, Parent)
        ...
        local Main = CoreGui:FindFirstChild('Cats.lol | CMD').Main
        ...
        Frame.InputChanged:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseMovement then DraggingInput = Input end
        end)
        UserInputService.InputChanged:Connect(function(Input)
            if Input == DraggingInput and Dragging then Update(Input) end
        end)
    end
    function Utility:StringToKeyCode(String)
        local Byte = string.byte(String)
        for _, KeyCode in next, Enum.KeyCode:GetEnumItems() do if KeyCode.Value == Byte then return KeyCode end end
    end
    function Utility:KeyCodeToString(KeyCode)
        if KeyCode.Value < 127 and KeyCode.Value > 33 then return string.char(KeyCode.Value) else return KeyCode.Name end
    end
    function Utility:GetProperty(Level, Name, Properties)
        local AllProperties = {Window = {Name = {'Name', 'NAME', 'name'}, IntroText = {'IntroText', 'INTROTEXT', 'introText', 'introtext', 'Introtext'}, IntroIcon = {'IntroIcon', 'INTROICON', 'introIcon', 'introicon', 'Introicon'}, IntroBlur = {'IntroBlur', 'Introblur', 'INTROBLUR', 'introBlur', 'introblur'}, IntroBlurIntensity = {'introblurintensity', 'Introblurintensity', 'INTROBLURINTENSITY', 'introBlurintensity', 'IntroBlurintensity', 'introblurIntensity', 'IntroblurIntensity', 'introBlurIntensity', 'IntroBlurIntensity'}, Theme = {'Theme', 'THEME', 'theme'}, Position = {'Position', 'position', 'POSITION'}, Draggable = {'Draggable', 'DRAGGABLE', 'draggable'}, Prefix = {'Prefix', 'PREFIX', 'prefix'}}}
        local PossibleNames = AllProperties[Level][Name]
        for _, Name in next, PossibleNames do if Properties[Name] then return Properties[Name] end end
    end
    function Utility:Destroy()
        BreakAllLoops = true
        for _, UI in next, CoreGui:GetChildren() do
            if UI.Name == 'Cats.lol | CMD' then for _, Item in next, UI.Main:GetChildren() do if Item.Name ~= 'MainCorner' and Item.Name ~= 'MainStroke' then Item:Destroy() end end
...
Library.Themes = {
    ['dark'] = {BackgroundColor = Color3.fromRGB(40, 40, 40), SecondaryColor = Color3.fromRGB(50, 50, 50), AccentColor = Color3.fromRGB(100, 100, 100), PrimaryTextColor = Color3.fromRGB(255, 255, 255), SecondaryTextColor = Color3.fromRGB(175, 175, 175)},
    ['light'] = {BackgroundColor = Color3.fromRGB(255, 255, 255), SecondaryColor = Color3.fromRGB(225, 225, 225), AccentColor = Color3.fromRGB(125, 125, 125), PrimaryTextColor = Color3.fromRGB(0, 0, 0), SecondaryTextColor = Color3.fromRGB(75, 75, 75)},
    ['syx'] = {BackgroundColor = Color3.fromRGB(50, 50, 50), PrimaryTextColor = Color3.fromRGB(255, 255, 255), SecondaryTextColor = Color3.fromRGB(125, 125, 125), AccentColor = Color3.fromRGB(70, 70, 70), SecondaryColor = Color3.fromRGB(65, 65, 65)},
    ['sw'] = {BackgroundColor = Color3.fromRGB(30, 30, 30), PrimaryTextColor = Color3.fromRGB(0, 125, 255), SecondaryTextColor = Color3.fromRGB(255, 255, 255), AccentColor = Color3.fromRGB(0, 125, 255), SecondaryColor = Color3.fromRGB(45, 45, 45)}
}
Library.Prefix = Utility:StringToKeyCode(';')
Library.Theme = nil
function Library:CreateWindow(Properties)
    local Name = Utility:GetProperty('Window', 'Name', Properties) or 'Cats.lol | CMD'
    local IntroText = Utility:GetProperty('Window', 'IntroText', Properties) or 'Cats.lol CMD UI'
    ...
    Library.Theme = Theme
    if type(Theme) == 'table' then Library.Themes['custom'] = Theme end
    Utility:Destroy()
    ...
    if Draggable then Utility:EnableDragging(Main, Main) end
    MainStroke.Thickness = 0
    if IntroBlur == true then
        Utility:Create('BlurEffect', {Name = 'CatslolIntroBlur', Parent = Lighting, Size = 0})
        local Blur = Lighting.CatslolIntroBlur
        Utility:Tween(Blur, {Size = IntroBlurIntensity}, 1)
        task.wait(1)
    end
if ismobile then
    local OpenBtn = Utility:Create('ImageButton', {Name = 'MobileOpenButton', Parent = Container, BackgroundTransparency = 1, Image = 'rbxassetid://3926305904', ImageColor3 = Library.Theme.PrimaryTextColor, Size = UDim2.new(0, 44, 0, 44), Position = UDim2.new(1, -55, 0, 10), AnchorPoint = Vector2.new(1, 0), ZIndex = 200}, {Utility:Create('UICorner', { CornerRadius = UDim.new(0, 8) })})
    task.spawn(function()
        while task.wait() do if ChangeTheme and not BreakLoops then Utility:Tween(OpenBtn, {ImageColor3 = Library.Theme.PrimaryTextColor}, 0.25) end end
    end)
    OpenBtn.MouseButton1Click:Connect(function()
        local fakeInput = {KeyCode = Library.Prefix, UserInputType = Enum.UserInputType.Keyboard}
        UserInputService.InputBegan:Fire(fakeInput, false)
    end)
    Main:GetPropertyChangedSignal('Position'):Connect(function()
        local barOpen = Main.Position.Y.Offset == 36 or Main.Position.Y.Offset == -72
        OpenBtn.Visible = not barOpen
    end)
    Main.CommandInput.PlaceholderText = 'open'
end
    ...
    task.wait(0.5)
    if IntroBlur == true then
        Utility:Tween(Lighting.CatslolIntroBlur, {Size = 0}, 1)
        task.wait(1)
    end
    Main['IntroText']:Destroy()
    Main['IntroImage']:Destroy()
    if IntroBlur == true then Lighting.CatslolIntroBlur:Destroy() end
    ...
    for _, Element in next, Main:GetChildren() do
        if Element:IsA('TextBox') or Element:IsA('TextLabel') or Element:IsA('TextButton') or Element:IsA('ImageButton') then
            ...
            if Utility:HasProperty(Element, 'BackgroundTransparency') then Utility:Tween(Element, {BackgroundTransparency = 0}, 0.25) end
            if Utility:HasProperty(Element, 'TextTransparency') then Utility:Tween(Element, {TextTransparency = 0}, 0.25) end
            if Utility:HasProperty(Element, 'ImageTransparency') then Utility:Tween(Element, {ImageTransparency = 0}, 0.25) end
        end
    end
    ...
    local function UpdateFrameSizes()
        local Count = 0
        for _, Instance in next, CommandsHolderScrolling:GetChildren() do if not Instance:IsA('UIListLayout') and Instance.Visible then Count += 1 end end
        local Size = Count * 30
        ...
        elseif Count < 9 then
            ...
            Utility:Tween(CommandsHolderScrolling, {Size = UDim2.new(0, 500, 0, Size)}, 0.25)
            Utility:Tween(CommandsHolderScrolling, {CanvasSize = UDim2.new(0, 500, 0, Size)}, 0.25)
            if string.find(Position, 'bottom') then if Main.Position.Y == UDim.new(1, 1) or Main.Position.Y == UDim.new(1, 0) or Main.Position.Y == UDim.new(1, 2) or Main.Position.Y == UDim.new(0, -37) then Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, -Main.Position.Y.Offset - Size - 13)}, 0.25) else Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, -Main.Position.Y.Offset - Size + 22)}, 0.25) end else if Main.Position.Y == UDim.new(1, 36) or Main.Position.Y == UDim.new(0, -72) then Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, -Main.Position.Y.Offset - 3)}, 0.25) else Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, -Main.Position.Y.Offset + 33)}, 0.25) end end
        else
            ...
            Utility:Tween(CommandsHolderScrolling, {CanvasSize = UDim2.new(0, 500, 0, Size)}, 0.25)
            if string.find(Position, 'bottom') then if Main.Position.Y == UDim.new(1, 1) or Main.Position.Y == UDim.new(1, 0) or Main.Position.Y == UDim.new(1, 2) or Main.Position.Y == UDim.new(0, -37) then Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, -Main.Position.Y.Offset - 273)}, 0.25) else Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, -Main.Position.Y.Offset - 237)}, 0.25) end else if Main.Position.Y == UDim.new(1, 36) or Main.Position.Y == UDim.new(0, -72) then Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, -Main.Position.Y.Offset - 3)}, 0.25) else Utility:Tween(CommandsHolder, {Position = UDim2.new(0, 0, 0, -Main.Position.Y.Offset + 33)}, 0.25) end end
        end
    end
                    ...
                    if string.find(Position, 'bottom') then
                        ...
                        for _, Instance in next, CommandsHolderScrolling:GetDescendants() do
                            if not Instance:IsA('UIListLayout') and not Instance:IsA('TextButton') then
                                if Instance:IsA('Frame') then Utility:Tween(Instance, {BackgroundTransparency = 0.25}, 0.25) end
                                if Utility:HasProperty(Instance, 'TextTransparency') then Utility:Tween(Instance, {TextTransparency = 0}, 0.25) end
                            end
                        end
                    else
                        Utility:Tween(Main, {Position = Main.Position + UDim2.new(0, 0, 0, 36)}, 0.25) 
    ...
    local function Execute(String)
        ...
        CommandInput.Text = ''
        local Command = assert(Commands[First], '[Cats.lol] Command Not Found: ' .. First)
        local NumberOfArguments = #Command.Arguments
        for Index, Argument in next, Split do if Index > 1 then table.insert(Arguments, Argument) end end
        if NumberOfArguments > #Arguments then
            local Missing = ''
            for Index = 1, NumberOfArguments do if not Arguments[Index] then Missing = Missing .. tostring(Command.Arguments[Index]) end end
            ...
            if not HoverDebounce then
                if Main.Position.Y == UDim.new(1, 0) or Main.Position.Y == UDim.new(0, -36) then
                    ...
                    if string.find(Position, 'bottom') then
                        for _, Instance in next, CommandsHolderScrolling:GetDescendants() do
                            if not Instance:IsA('UIListLayout') and not Instance:IsA('TextButton') then
                                if Instance:IsA('Frame') then Utility:Tween(Instance, {BackgroundTransparency = 1}, 0.25) end
                                if Utility:HasProperty(Instance, 'TextTransparency') then Utility:Tween(Instance, {TextTransparency = 1}, 0.25) end
                            end
                        end
                        task.wait(0.25)
                        Utility:Tween(Main, {Position = Main.Position + UDim2.new(0, 0, 0, 36)}, 0.25)
    ...
    CommandInput:GetPropertyChangedSignal('Text'):Connect(function()
        if CommandInput.Text == '' then
            ...
            for Index, String in next, Split do
                if Index == 1 then Full = Full .. Split[1]:gsub(Utility:KeyCodeToString(Library.Prefix), '') else Full = Full .. Split[Index] end
            end
    ...
    function WindowFunctions:GetThemes(Names)
        if Names then
            local Table = {}
            for Index, Theme in next, Library.Themes do table.insert(Table, Index) end
            return Table
        else
    ...
    function WindowFunctions:CreateNotification(Title, Text, Duration)
        local function NotificationCount()
            local Amount = 0
            for _, Notification in next, Container:GetChildren() do if string.find(Notification.Name, 'Notification') then Amount += 1 end end 
            return Amount
        end
        task.wait(0.5)
        task.spawn(function()
    ...
    function WindowFunctions:AddCommand(Name, Arguments, Description, Callback)
        ...
        local function ConstructString(Highlighted)
            if Highlighted then
                ...
                local String = Name .. ' '
                for _, Argument in next, Arguments do String = String .. '{' .. Argument .. '}' .. ' ' end
